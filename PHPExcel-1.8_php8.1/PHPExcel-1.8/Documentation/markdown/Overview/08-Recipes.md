...
use app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExcelrequire_once 'PHPExcel/Cell/AdvancedValueBinder.php';
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()
...
```php
use app\PHPExcel$formula = $objPHPExcel->getActiveSheet()->getCell('B8')->getValue();
...
```php
use app\PHPExcel$formula = '=????360(????(2010;2;5);????(2010;12;31);??????)';
...
```php
use app\PHPExceluse app\PHPExcelrequire_once 'PHPExcel/Cell/AdvancedValueBinder.php';
PHPExcel_Cell::setValueBinder( new PHPExcel_Cell_AdvancedValueBinder() );
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->getCell('A1')
    ->setValueExplicit(
        '25', 
...
```php
use app\PHPExceluse app\PHPExcel$objPHPExcel->getActiveSheet()->getPageSetup()
...
```php
use app\PHPExceluse app\PHPExcel$objDrawing = new PHPExcel_Worksheet_HeaderFooterDrawing();
...
```php
use app\PHPExcel$objPHPexcel = PHPExcel_IOFactory::load('test.xlsx');
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->setBreak( 'A10' , PHPExcel_Worksheet::BREAK_ROW );
```

The following line of code sets a print break on column D:

```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->setBreak( 'D10' , PHPExcel_Worksheet::BREAK_COLUMN );
```
...
```php
use app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExcel$objPHPExcel->getActiveSheet()->getStyle('B2')
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->getStyle('B3:B7')->getFill()
...
```php
use app\PHPExceluse app\PHPExceluse app\PHPExcel$styleArray = array(
    'font' => array(
        'bold' => true,
    ),
    'alignment' => array(
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->getStyle('A1')->getNumberFormat()
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->getStyle('A1:D4')
...
```php
use app\PHPExcel$styleArray = array(
    'borders' => array(
        'outline' => array(
...
```php
use app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExcel$objConditional1 = new PHPExcel_Style_Conditional();
...
```php
use app\PHPExcel$objPHPExcel->getActiveSheet()->getStyle('B1')
    ->getProtection()
...
```php
use app\PHPExceluse app\PHPExcel$objValidation = $objPHPExcel->getActiveSheet()->getCell('B3')
    ->getDataValidation();
...
```php
use app\PHPExceluse app\PHPExcel$objValidation = $objPHPExcel->getActiveSheet()->getCell('B5')
    ->getDataValidation();
...
```php
use app\PHPExcel$objDrawing = new PHPExcel_Worksheet_Drawing();
...
use app\PHPExceluse app\PHPExceluse app\PHPExcel$gdImage = @imagecreatetruecolor(120, 20) or die('Cannot Initialize new GD image stream');
...
```php
use app\PHPExceluse app\PHPExceluse app\PHPExceluse app\PHPExcel$i = 0;
...
```php
use app\PHPExceluse app\PHPExceluse app\PHPExcel$objRichText = new PHPExcel_RichText();
...
```php
// Add some data
use app\PHPExceluse app\PHPExcel$objPHPExcel->setActiveSheetIndex(0);
...
use app\PHPExcelheader('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
// redirect output to client browser
use app\PHPExcelheader('Content-Type: application/vnd.ms-excel');
