    ...
    $('.searchmain').keyup(function () {
        if ($(this).val().length >= 2) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log(data.query_nomenclature[0]);
                    let tbody = $('.searchbody');
                    tbody.empty();
                    var $maxRows = Math.max(data.query_nomenclature.length, data.query_users.length, data.query_clients.length);
                    for (var i = 0; i < $maxRows; i++) {
                        var html_ =
                            '<tr>' +
                            '<td>' + (data.query_nomenclature[i] && data.query_nomenclature[i].name ? data.query_nomenclature[i].name : '') + '</td>' +
                            '<td>' + (data.query_users[i] && data.query_users[i].name ? data.query_users[i].name : '') + '</td>' +
                            '<td>' + (data.query_clients[i] && data.query_clients[i].name ? data.query_clients[i].name : '') + '</td>' +
                            '</tr>';
                        tbody.append(html_);
                    }
                }
